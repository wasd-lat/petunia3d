...
| ISSUE-026 | HIGH | Docs | `docs/generated/COMMANDS.md` estava defasado em 14 comandos e o gerador emite linhas duplicadas para `edit.delete` e `edit.duplicate`. | PARTIAL — a defasagem foi corrigida nesta linha (regeneração); as linhas duplicadas continuam e apontam para registro duplicado no `CommandDispatcher`, que precisa ser investigado antes de mexer no gerador. |
