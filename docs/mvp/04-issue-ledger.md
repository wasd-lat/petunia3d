...
| ISSUE-009 | HIGH | Paint | Sem UI de layers, effects, fill scope, lock, projeção, formas e canvas 2D. | PARTIAL — painel de camadas completo (adicionar, grupo, ativar, visibilidade, lock, reordenar, remover com guarda da base, opacidade) ligado ao stack canônico e recompondo o raster. Faltam effects, fill scope, projeção, formas e canvas 2D. |
