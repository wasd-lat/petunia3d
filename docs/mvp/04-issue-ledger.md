...
| ISSUE-009 | HIGH | Paint | Sem UI de layers, effects, fill scope, lock, projeção, formas e canvas 2D. | PARTIAL — camadas completas; **fill scope** (ConnectedPixels/Face/SelectedFaces/UvIsland/Object) com flood fill e rasterização de polígono UV; **projeção** Surface/ScreenSpace; **trava de pincel** None/FirstObject/FirstFace; **Line e Rectangle** ancorados com commit no release. Faltam effects (camada de efeito), canvas 2D visível e pintura por pixel. |
| ISSUE-010 | HIGH | UV | Sem UV Editor 2D; seams e diagnósticos inacessíveis. | PARTIAL — editor 2D desenha o contorno real das UVs na caixa 0..1 com contagem de ilhas e face selecionada, e as ferramentas de costura marcam/desmarcam/limpam com undo. Faltam seleção direta no editor 2D, mover/rotacionar/escalar UVs pelo editor e visualização de ilhas por cor. |
