...
| ISSUE-008 | HIGH | Model | Extrude/Inset/Bevel/Extrude Individual/Scale Selection one-shot. | PARTIAL — Extrude, Extrude Individual, Inset, Bevel, Push/Pull e Scale Selection são sessões paramétricas com preview, Tool Properties e undo único. Fuse/Cut/Intersect, **Join** e o modificador **Keep Parts** entraram como comandos canônicos com operando explícito. `Connect` existe via palette exigindo exatamente duas faces selecionadas. Faltam Draw/Profile e Bridge como fluxos de UI. |
| ISSUE-009 | HIGH | Paint | Sem UI de layers, effects, fill scope, lock, projeção, formas e canvas 2D. | PARTIAL — camadas completas; fill scope (5 escopos) com flood fill e rasterização de polígono UV; projeção Surface/ScreenSpace; trava de pincel; Line e Rectangle ancorados; canvas 2D visível com os pixels reais; **camadas de efeito** (6 tipos) com parâmetros expostos e limitados à faixa do efeito. Falta a grade de pixels e a camada de decalque como fluxo de UI. |
