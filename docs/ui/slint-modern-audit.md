...
| Navegação única de workspace | `PARTIALLY_COMPLIANT` | MODEL/PAINT/UV vivem somente nos pills do header; resta validar foco e densidade do header em janelas estreitas |
...
| Numeric field | `PARTIALLY_COMPLIANT` | scrubbing transacional, fine-step, clamp e commit textual funcionam; resta validar foco e edição por locale na janela |
...
| Integração GPU Slint/viewport | `PARTIALLY_COMPLIANT` | o Slint usa a configuração WGPU 30 compartilhada e importa a textura do viewport com `slint::Image::try_from`; faltam benchmarks do fast path e validação em hardware heterogêneo |
...
| AccessKit | `COMPLIANT` | feature `accessibility` ativa; papéis e rótulos semânticos em `TopAction`, `ToolButton`, `NumericField`, etc. |
...
| Atalhos de teclado globais | `PARTIALLY_COMPLIANT` | o shell roteia eventos físicos para `Keybinds` e preserva Tab para navegação de foco; falta cobertura completa dos oito perfis na janela |
...
| Keymap profiles (8 perfis canônicos) | `PARTIALLY_COMPLIANT` | o shell Slint roteia eventos através de `Keybinds`; falta validação sistemática por perfil na janela |
| i18n TOML (`pt-BR.toml`, `en.toml`) | `MISSING` | Arquivos existem em `assets/locales/` para o legado egui; não portados para o Slint |
| Fast path GPU sem readback | `PARTIALLY_COMPLIANT` | `viewport_gpu.rs` importa a textura WGPU compartilhada diretamente; faltam benchmarks de frame e testes de regressão em hardware heterogêneo |
...
shortcut routing, viewport picking for Object/Point/Face selection, numeric text
commit, command-palette keyboard selection, disabled command reasons,
paint fill/canonical UV dispatch, responsive inspector collapse, a pinned Parts
drawer, semantic viewport tokens, and the modular `tokens.slint`/`input.rs`
boundaries.
