...
> **Frontend de Produção:** `petunia_ui_slint` ([`crates/ui-slint/`](../../crates/ui-slint/))  
...
| :--- | :---: | :---: | :---: | :---: |
| **Modelagem (MODEL)** | 45 | 33 (73.3%) | 10 (22.2%) | 2 (4.4%) |
| **Pintura, UV e Materiais (PAINT)** | 30 | 19 (63.3%) | 9 (30.0%) | 2 (6.7%) |
| **Total Consolidado** | **75** | **52 (69.3%)** | **19 (25.3%)** | **4 (5.3%)** |
...
| :---: | :--- | :--- | :---: | :--- |
| **1** | **Seleção de Vértices (Point Select)** | [P3D-017](../bible/specs/p3d-017-vertex-select.md) | **Implementado** | Discos visuais com hover e highlight nos renderizadores WGPU e software; clique e Shift+clique em [`crates/core/src/selection.rs`](../../crates/core/src/selection.rs). |
...
| **6** | **Seleção por Laço (Lasso Select)** | [P3D-020](../bible/specs/p3d-020-object-selection.md) | **Parcialmente** | Algoritmo de polígono 2D e ferramenta criados em [`crates/ui-slint/src/lib.rs`](../../crates/ui-slint/src/lib.rs); teste de picking interno e oclusão profunda pendentes de QA. |
| **7** | **Seleção de Loop de Arestas (Loop Select)** | [Capítulo 03](../bible/foundations/03-geometry-core-faces-topologia.md) | **Parcialmente** | `LoopRing::discover` e `select_edge_loop` operam no Core ([`crates/mesh/src/loop_cut.rs`](../../crates/mesh/src/loop_cut.rs)); atalho direto Alt+clique no Slint ainda requer calibração no picking. |
...
| **11** | **Universal Transform (Gizmo Combinado)** | [P3D-024](../bible/specs/p3d-024-universal-transform.md) | **Parcialmente** | Handles individuais de Move, Rotate e Scale funcionais; gizmo combinado triaxial adicionado na UI Slint, mas picking unificado sem conflito de handles segue em refinamento. |
...
| **14** | **Sistema de Pivô (Pivot System)** | [P3D-027](../bible/specs/p3d-027-pivot-system.md) | **Parcialmente** | Modos Median Point, Bounding Box Center, 3D Cursor e Individual Origins no Core; selector adicionado na UI Slint, com validação de seleção múltipla em andamento. |
...
| **16** | **Primitivas Básicas (Cubo, Cilindro, Esfera, Icoesfera, Cone, Plano)** | [P3D-028](../bible/specs/p3d-028-add-primitives.md) | **Implementado** | Geradores low-poly em [`crates/mesh/src/primitives.rs`](../../crates/mesh/src/primitives.rs) com botões dedicados na barra lateral esquerda da UI Slint. |
...
| **19** | **Desenho de Perfil 2D (Draw Profile: Polyline, Retângulo, Círculo)** | [Capítulo 06](../bible/foundations/06-escopo-essencial.md), [14](../bible/foundations/14-modelagem-v1-cut-slice-bevel-revolve-sweep.md) | **Parcialmente** | `DrawProfileTool` permite desenhar polylines, fechar perfis e gerar volumes via Extrude/Revolve no Slint. Atalhos para retângulos/círculos prontos ainda não expostos diretamente. |
...
| **26** | **Slice (Corte por Plano Infinito)** | [Capítulo 14](../bible/foundations/14-modelagem-v1-cut-slice-bevel-revolve-sweep.md) | **Parcialmente** | Atalho `Shift+K`; divide geometria preservando ambos os lados por padrão. O preview atual exibe a linha do plano; preview da malha cortada antes do release segue pendente. |
...
| **33** | **Proportional Editing (Edição Proporcional)** | [P3D-039](../bible/specs/p3d-039-proportional-editing.md) | **Parcialmente** | Kernel de falloff euclidiano funcional no Core ([`crates/core/src/proportional.rs`](../../crates/core/src/proportional.rs)); ajuste interativo do raio durante o gesto no Slint requer polimento. |
| **34** | **Sistema de Snapping (Grade e Elementos)** | [P3D-040](../bible/specs/p3d-040-sistema-de-snap.md) | **Parcialmente** | Snapping magnético à grade métrica e a vértices ativo em transformações modais; snapping a arestas/faces não implementado. |
| **35** | **Dissolve (Remoção Segura de Vértices e Arestas)** | [Capítulo 06](../bible/foundations/06-escopo-essencial.md), [14](../bible/foundations/14-modelagem-v1-cut-slice-bevel-revolve-sweep.md) | **Implementado** | Remoção de edges/points coplanares sem destruir as faces adjacentes ([`crates/mesh/src/ops.rs`](../../crates/mesh/src/ops.rs)). |
...
| **38** | **Combine: Fuse (União Booleana 3D)** | [Capítulo 04](../bible/foundations/04-combine-fuse-weld-personagens.md), [12](../bible/foundations/12-baseline-funcional-roadmap-escopo.md) | **Implementado** | União booleana volumétrica via provider robusto `manifold-rust` ([`crates/mesh/src/boolean.rs`](../../crates/mesh/src/boolean.rs)). |
...
| **43** | **Orientação de Normais e Recálculo** | [Capítulo 06](../bible/foundations/06-escopo-essencial.md), [12](../bible/foundations/12-baseline-funcional-roadmap-escopo.md) | **Parcialmente** | `FlipNormalsCmd` e `recalculate_normals` operam no Core; overlay visual colorido (Face Orientation) ainda não integrado no WGPU. |
...
| **2** | **Soft Brush (Pincel Suave)** | [P3D-057](../bible/specs/p3d-057-soft-brush.md) | **Implementado** | Atenuação circular quadrática com hardness e size configuráveis em [`crates/module-paint/src/lib.rs`](../../crates/module-paint/src/lib.rs). |
| **3** | **Airbrush (Aerógrafo Contínuo)** | Iniciativa Paint 2026-09-16, [Cap. 44](../bible/foundations/44-pos-v1-surface-paint-toolbox.md) | **Parcialmente** | `BrushSettings` contém os parâmetros de `flow` e `spacing` no Core; stroke temporizado contínuo pendente no shell Slint. |
...
| **7** | **Formas Raster (Linha e Retângulo)** | [Capítulo 15](../bible/foundations/15-uv-textura-projecao-materiais.md) | **Parcialmente** | `ShapeStroke` implementado em [`crates/module-paint/src/lib.rs`](../../crates/module-paint/src/lib.rs); botões de traçado de linha/retângulo ainda não expostos no Slint. |
...
| **11** | **Editor 2D de Textura (Paint 2D Canvas)** | [Capítulo 12](../bible/foundations/12-baseline-funcional-roadmap-escopo.md), [15](../bible/foundations/15-uv-textura-projecao-materiais.md) | **Parcialmente** | Pré-visualização da textura exibida no painel de Paint (`paint-canvas-image`); canvas 2D interativo com zoom/pan e pintura completa no Slint em desenvolvimento. |
| **12** | **Pixel Grid Contextual** | [Capítulo 06](../bible/foundations/06-escopo-essencial.md), [15](../bible/foundations/15-uv-textura-projecao-materiais.md) | **Não implementado** | Grid overlay de demarcação de texels individuais sob alto zoom ainda não conectado na UI Slint. |
...
| **15** | **Decal & Projection Layers** | [P3D-133](../bible/specs/p3d-133-decal-projection-layers.md) | **Parcialmente** | Estrutura `DecalLayer` com UV, escala, rotação e opacidade no Core; manipulador 3D de superfície na viewport previsto para a Era 1 pós-GA. |
...
| **18** | **Importação e Exportação de Paletas (.gpl / .hex)** | [Capítulo 06](../bible/foundations/06-escopo-essencial.md), [15](../bible/foundations/15-uv-textura-projecao-materiais.md) | **Parcialmente** | `ProjectService::import_palette` e `export_palette` prontos e testados no Core; botões de diálogo in-canvas ainda não expostos no menu de Paint do Slint. |
...
| **22** | **Project From Reference / View** | [Capítulo 08](../bible/foundations/08-photo-projection-trace-project.md), [15](../bible/foundations/15-uv-textura-projecao-materiais.md) | **Parcialmente** | `UvProjectFromViewCmd` projeta a vista da câmera; integração do bake automático da imagem de referência carregada na textura ainda em polimento. |
| **23** | **Transformações de Ilhas UV (Move, Scale, Rotate 90°)** | [P3D-064](../bible/specs/p3d-064-uv-editing-basico.md), [Capítulo 15](../bible/foundations/15-uv-textura-projecao-materiais.md) | **Parcialmente** | Operações implementadas em [`crates/module-uv/src/lib.rs`](../../crates/module-uv/src/lib.rs); workspace UV dedicado foi temporariamente condensado no shell Slint seguindo o fluxo Paint-first ([decisão 2026-09-16](../../PROJECT_STATE.md#L228)). |
| **24** | **Costuras UV Manuais (UV Seams - Mark / Clear)** | [Capítulo 15](../bible/foundations/15-uv-textura-projecao-materiais.md) | **Parcialmente** | `Mesh::mark_seam`, `clear_seam` e `toggle_seam` implementados em [`crates/mesh/src/uv_tools.rs`](../../crates/mesh/src/uv_tools.rs); toggle via atalho na UI Slint ainda não conectado. |
...
| **26** | **Diagnóstico de Distorção UV (Checker & Stretch)** | [Capítulo 06](../bible/foundations/06-escopo-essencial.md), [15](../bible/foundations/15-uv-textura-projecao-materiais.md) | **Parcialmente** | `UvDiagnostics` calcula área zero, sobreposições e stretch médio/máximo; shader do mapa xadrez (Checkerboard) na viewport Slint pendente de ativação direta. |
...
| **29** | **Atribuição de Múltiplos Materiais por Face (Material Slots)** | [P3D-050](../bible/specs/p3d-050-material-system.md), [Capítulo 15](../bible/foundations/15-uv-textura-projecao-materiais.md) | **Parcialmente** | `Face.material_slot` suportado no modelo de dados e exportador; lista visual e seletor de slots por face no novo Inspector direito Slint ainda em revisão. |
...
1. **Modelagem:**
   - O núcleo geométrico em [`crates/mesh/`](../../crates/mesh/) e transacional em [`crates/core/`](../../crates/core/) está praticamente completo e verificado por testes de regressão automatizados.
...
2. **Pintura e UV:**
   - O motor raster em [`crates/module-paint/`](../../crates/module-paint/) e projeção UV em [`crates/module-uv/`](../../crates/module-uv/) já entregam camadas com blend modes, pintura 3D baricêntrica contínua, isolamento de seleção, xatlas e efeitos base.
   - Conforme a decisão de produto de 2026-09-16 registrada em [`PROJECT_STATE.md`](../../PROJECT_STATE.md#L221-L244), o fluxo prioritário é **Paint-first**: consolidar o editor 2D integrado e o descriptor unificado de pincéis antes de reabrir o redesign visual do workspace UV independente.
