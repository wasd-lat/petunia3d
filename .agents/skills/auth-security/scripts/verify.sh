...
SKILL_DIR="src/prumo/resources/workforce/skills/auth-security"

echo "[Prumo Skill: auth-security] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/authentication-cheatsheet.md" \
  "templates/auth-security-report.md" \
  "checks/auth-security-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "auth-security" <<'PY'
import json
import os
import sys
...
check_grep "PKCE|OAuth" "SKILL.md" "skill covers federated authentication flow"
check_grep "Argon2id|password" "SKILL.md" "skill covers password storage"
check_grep "refresh token|revocation" "SKILL.md" "skill covers token rotation and revocation"
check_grep "Session Management|PKCE" "references/authentication-cheatsheet.md" "reference covers sessions and OAuth"
check_grep "refresh tokens|revocation" "checks/auth-security-checklist.md" "checklist covers token lifecycle"
check_grep "Customer Session|AUTH-01" "templates/auth-security-report.md" "template contains a realistic identity audit"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] auth-security verification passed"
  exit 0
fi

echo "[ERROR] auth-security verification failed with $VIOLATIONS violation(s)"
exit 1
