...
SKILL_DIR="src/prumo/resources/workforce/skills/game-runtime"

echo "[Prumo Skill: game-runtime] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/game-runtime-guide.md" \
  "templates/game-runtime-template.md" \
  "checks/game-runtime-invariants.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "game-runtime" <<'PY'
import json
import os
import sys
...
check_grep "fixed timestep|accumulator" "SKILL.md" "skill requires a bounded fixed-timestep loop"
check_grep "interpolat|scene" "SKILL.md" "skill covers interpolation and scene transitions"
check_grep "device loss|resume" "SKILL.md" "skill covers hostile lifecycle events"
check_grep "Fixed Simulation Step|Determinism" "references/game-runtime-guide.md" "reference defines tick and determinism contracts"
check_grep "Tick Contract|Determinism" "checks/game-runtime-invariants.md" "checklist covers tick and determinism gates"
check_grep "Fixed-Timestep|Accumulator" "templates/game-runtime-template.md" "template contains a realistic runtime report"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] game-runtime verification passed"
  exit 0
fi

echo "[ERROR] game-runtime verification failed with $VIOLATIONS violation(s)"
exit 1
