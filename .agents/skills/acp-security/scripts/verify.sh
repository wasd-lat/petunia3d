...
SKILL_DIR="src/prumo/resources/workforce/skills/acp-security"

echo "[Prumo Skill: acp-security] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/acp-security-guide.md" \
  "templates/acp-security-report.md" \
  "checks/acp-security-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "acp-security" <<'PY'
import json
import os
import sys
...
check_grep "delegation|capability" "SKILL.md" "skill constrains capability delegation"
check_grep "replay|sequence" "SKILL.md" "skill covers message integrity and replay defense"
check_grep "quarantine|isolation" "SKILL.md" "skill covers untrusted agent isolation"
check_grep "Trust Model|replay" "references/acp-security-guide.md" "reference defines ACP trust and replay controls"
check_grep "delegation depth|handoff" "checks/acp-security-checklist.md" "checklist covers delegation and handoff security"
check_grep "ACP-01|Research Worker" "templates/acp-security-report.md" "template contains a realistic ACP audit"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] acp-security verification passed"
  exit 0
fi

echo "[ERROR] acp-security verification failed with $VIOLATIONS violation(s)"
exit 1
