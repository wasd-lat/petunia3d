...
PY
  then
    pass "Manifest and asset validation completed."
  else
    fail "Manifest or asset validation failed."
  fi
else
...
check_grep "physics|collision" "SKILL.md" "SKILL.md names the physics and collision domain"
check_grep "fixed timestep|determin" "SKILL.md" "SKILL.md covers deterministic fixed-step simulation"
check_grep "broadphase|narrowphase|solver" "SKILL.md" "SKILL.md names the collision pipeline"
check_grep "core concepts|patterns|anti-patterns" "references/physics-collision-guide.md" "Reference guide has domain patterns"
check_grep "- \[ \]" "checks/physics-collision-invariants.md" "Checklist contains verification items"
check_grep "verification evidence|handoff" "templates/physics-collision-template.md" "Template contains concrete evidence fields"
