...
SKILL_DIR="src/prumo/resources/workforce/skills/implementation-reality-verification"

echo "[Prumo Skill: implementation-reality-verification] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/implementation-reality-verification-guide.md" \
  "templates/implementation-reality-verification-spec.md" \
  "checks/implementation-reality-verification-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "implementation-reality-verification" <<'PY'
import json
import os
import sys
...
check_grep "Presence|Reachability" "SKILL.md" "skill separates presence from reachability"
check_grep "false-green|oracle" "SKILL.md" "skill detects false-green tests"
check_grep "Terminal State|accepted" "SKILL.md" "skill defines a terminal-state hierarchy"
check_grep "Terminal State Hierarchy|False-Green" "references/implementation-reality-verification-guide.md" "reference defines verification states and false greens"
check_grep "Reachability|False-Green" "checks/implementation-reality-verification-checklist.md" "checklist covers reachability and oracle strength"
check_grep "Atomic Upload|Reachability" "templates/implementation-reality-verification-spec.md" "template contains a realistic reality report"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] implementation-reality-verification verification passed"
  exit 0
fi

echo "[ERROR] implementation-reality-verification verification failed with $VIOLATIONS violation(s)"
exit 1
