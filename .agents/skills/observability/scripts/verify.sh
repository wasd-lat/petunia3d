...
SKILL_DIR="src/prumo/resources/workforce/skills/observability"

echo "[Prumo Skill: observability] Asset verification"
VIOLATIONS=0
...
for file in \
  ...
  "checks/observability-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "observability" <<'PY'
import json
import os
import sys
...
check_grep "Structured Logging|tracing" "SKILL.md" "skill requires structured logs and traces"
check_grep "Correlation ID|correlation" "SKILL.md" "skill propagates correlation IDs"
check_grep "redaction|cardinality" "SKILL.md" "skill covers secret redaction and bounded labels"
check_grep "Structured Logging|Distributed Tracing" "references/structured-logging.md" "reference defines logging and tracing patterns"
check_grep "Correlation|redacted" "checks/observability-checklist.md" "checklist covers correlation and redaction"
check_grep "Checkout Telemetry|RED" "templates/observability-template.md" "template contains a realistic telemetry report"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] observability verification passed"
  exit 0
fi

echo "[ERROR] observability verification failed with $VIOLATIONS violation(s)"
exit 1
