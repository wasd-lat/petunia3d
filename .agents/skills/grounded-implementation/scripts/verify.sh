...
SKILL_DIR="src/prumo/resources/workforce/skills/grounded-implementation"

echo "[Prumo Skill: grounded-implementation] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/grounded-implementation-guide.md" \
  "templates/grounded-implementation-spec.md" \
  "checks/grounded-implementation-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "grounded-implementation" <<'PY'
import json
import os
import sys
...
check_grep "Inspect Before Claim" "SKILL.md" "skill requires inspection before claims"
check_grep "Never Invent|anti-invention" "SKILL.md" "skill prohibits invented APIs and requirements"
check_grep "Scope|Fail Closed" "SKILL.md" "skill enforces scope and fail-closed authority"
check_grep "UNKNOWN Protocol|Scope Boundaries" "references/grounded-implementation-guide.md" "reference defines the grounding protocol"
check_grep "Inspection Before Claim|Authority" "checks/grounded-implementation-checklist.md" "checklist covers inspection and authority"
check_grep "Checkout Retry|UNKNOWN" "templates/grounded-implementation-spec.md" "template contains a realistic grounded record"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] grounded-implementation verification passed"
  exit 0
fi

echo "[ERROR] grounded-implementation verification failed with $VIOLATIONS violation(s)"
exit 1
