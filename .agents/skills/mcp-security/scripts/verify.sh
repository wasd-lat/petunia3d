...
SKILL_DIR="src/prumo/resources/workforce/skills/mcp-security"

echo "[Prumo Skill: mcp-security] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/mcp-security-guide.md" \
  "templates/mcp-security-report.md" \
  "checks/mcp-security-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "mcp-security" <<'PY'
import json
import os
import sys
...
check_grep "Tool Execution|shell" "SKILL.md" "skill restricts MCP tool execution"
check_grep "Schema|argument" "SKILL.md" "skill validates tool arguments strictly"
check_grep "Prompt Injection|Protected Path" "SKILL.md" "skill covers injection and path protection"
check_grep "Tool Gateway|Untrusted Output" "references/mcp-security-guide.md" "reference defines gateway and injection boundaries"
check_grep "schema|protected|approval" "checks/mcp-security-checklist.md" "checklist covers schemas, paths, and approval"
check_grep "Workspace Tool|MCP-01" "templates/mcp-security-report.md" "template contains a realistic MCP audit"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] mcp-security verification passed"
  exit 0
fi

echo "[ERROR] mcp-security verification failed with $VIOLATIONS violation(s)"
exit 1
