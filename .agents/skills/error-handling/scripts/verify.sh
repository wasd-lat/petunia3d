...
SKILL_DIR="src/prumo/resources/workforce/skills/error-handling"

echo "[Prumo Skill: error-handling] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/error-handling-guidelines.md" \
  "templates/error-handling-template.md" \
  "checks/error-taxonomy-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "error-handling" <<'PY'
import json
import os
import sys
...
check_grep "taxonomy|typed" "SKILL.md" "skill requires a typed error taxonomy"
check_grep "retry|backoff" "SKILL.md" "skill bounds transient retries"
check_grep "context|user-facing" "SKILL.md" "skill separates diagnostic and user-facing errors"
check_grep "Result Pattern|Context" "references/error-handling-guidelines.md" "reference defines result and wrapping patterns"
check_grep "typed|resources" "checks/error-taxonomy-checklist.md" "checklist covers typing and cleanup"
check_grep "Payments Charge Path|Taxonomy" "templates/error-handling-template.md" "template contains a realistic error report"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] error-handling verification passed"
  exit 0
fi

echo "[ERROR] error-handling verification failed with $VIOLATIONS violation(s)"
exit 1
