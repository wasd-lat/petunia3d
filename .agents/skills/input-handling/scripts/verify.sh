...
SKILL_DIR="src/prumo/resources/workforce/skills/input-handling"

echo "[Prumo Skill: input-handling] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/input-handling-guide.md" \
  "templates/input-handling-template.md" \
  "checks/input-handling-invariants.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "input-handling" <<'PY'
import json
import os
import sys
...
check_grep "action mapping|Map Actions" "SKILL.md" "skill requires action-based mapping"
check_grep "buffer|dead zone" "SKILL.md" "skill covers bounded buffering and calibration"
check_grep "gamepad|hot-plug|chord" "SKILL.md" "skill covers multi-device behavior"
check_grep "Action Abstraction|Buffered Input" "references/input-handling-guide.md" "reference defines action and buffer patterns"
check_grep "Action Mapping|Analog" "checks/input-handling-invariants.md" "checklist covers action and analog contracts"
check_grep "Rebindable Action Map|Action map" "templates/input-handling-template.md" "template contains a realistic input report"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] input-handling verification passed"
  exit 0
fi

echo "[ERROR] input-handling verification failed with $VIOLATIONS violation(s)"
exit 1
