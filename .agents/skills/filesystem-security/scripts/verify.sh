...
SKILL_DIR="src/prumo/resources/workforce/skills/filesystem-security"

echo "[Prumo Skill: filesystem-security] Asset verification"
VIOLATIONS=0
...
for file in \
  ...
  "templates/filesystem-security-report.md" \
  "checks/filesystem-security-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "filesystem-security" <<'PY'
import json
import os
import sys
...
check_grep "traversal|symlink" "SKILL.md" "skill covers traversal and link attacks"
check_grep "TOCTOU|O_EXCL" "SKILL.md" "skill closes check-use race windows"
check_grep "atomic|permissions" "SKILL.md" "skill requires atomic and permission-safe writes"
check_grep "File Type Validation|Prevent Execution" "references/file-upload-security.md" "reference covers upload controls"
check_grep "TOCTOU|atomic|Protected" "checks/filesystem-security-checklist.md" "checklist covers path and mutation safety"
check_grep "Workspace Import|FS-01" "templates/filesystem-security-report.md" "template contains a realistic filesystem audit"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] filesystem-security verification passed"
  exit 0
fi

echo "[ERROR] filesystem-security verification failed with $VIOLATIONS violation(s)"
exit 1
