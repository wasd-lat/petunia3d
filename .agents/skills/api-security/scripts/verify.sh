...
SKILL_DIR="src/prumo/resources/workforce/skills/api-security"

echo "[Prumo Skill: api-security] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/owasp-api-top-10.md" \
  "templates/api-security-report.md" \
  "checks/api-security-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "api-security" <<'PY'
import json
import os
import sys
...
check_grep "BOLA|Object Level Authorization" "SKILL.md" "skill covers object-level authorization"
check_grep "Idempotency-Key|replay" "SKILL.md" "skill covers mutating-request replay protection"
check_grep "CORS|rate limit" "SKILL.md" "skill covers browser and resource controls"
check_grep "SSRF|Consumption" "references/owasp-api-top-10.md" "reference covers API risk classes"
check_grep "BOLA|RFC 7807" "checks/api-security-checklist.md" "checklist covers authorization and safe errors"
check_grep "Merchant Invoice|API-01" "templates/api-security-report.md" "template contains a realistic API audit"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] api-security verification passed"
  exit 0
fi

echo "[ERROR] api-security verification failed with $VIOLATIONS violation(s)"
exit 1
