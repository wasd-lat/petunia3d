...
SKILL_DIR="src/prumo/resources/workforce/skills/dependency-management"

echo "[Prumo Skill: dependency-management] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/dependency-injection-principles.md" \
  "templates/dependency-management-template.md" \
  "checks/dependency-audit.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "dependency-management" <<'PY'
import json
import os
import sys
...
check_grep "pin|lockfile" "SKILL.md" "skill requires deterministic pinning"
check_grep "advis|vulnerab" "SKILL.md" "skill covers vulnerability advisories"
check_grep "license|transitive" "SKILL.md" "skill covers licensing and transitive risk"
check_grep "Inversion of Control|Composition Root" "references/dependency-injection-principles.md" "reference explains dependency boundaries"
check_grep "vulnerabilities|licenses|pinned" "checks/dependency-audit.md" "checklist covers supply-chain gates"
check_grep "API Gateway|Reproducible" "templates/dependency-management-template.md" "template contains a realistic audit"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] dependency-management verification passed"
  exit 0
fi

echo "[ERROR] dependency-management verification failed with $VIOLATIONS violation(s)"
exit 1
