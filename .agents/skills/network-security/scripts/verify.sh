...
SKILL_DIR="src/prumo/resources/workforce/skills/network-security"

echo "[Prumo Skill: network-security] Asset verification"
VIOLATIONS=0
...
for file in \
  ...
  "templates/network-security-report.md" \
  "checks/network-security-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "network-security" <<'PY'
import json
import os
import sys
...
check_grep "TLS 1.3|certificate" "SKILL.md" "skill enforces modern TLS and certificate validation"
check_grep "egress|DNS rebinding" "SKILL.md" "skill governs egress and hostname validation"
check_grep "timeout|connection" "SKILL.md" "skill bounds client and server resources"
check_grep "Protocol Versions|Certificate Management" "references/tls-configuration.md" "reference defines TLS and certificate policy"
check_grep "TLS|timeout|WebSockets" "checks/network-security-checklist.md" "checklist covers transport and exhaustion controls"
check_grep "Merchant API Edge|NET-01" "templates/network-security-report.md" "template contains a realistic network audit"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] network-security verification passed"
  exit 0
fi

echo "[ERROR] network-security verification failed with $VIOLATIONS violation(s)"
exit 1
