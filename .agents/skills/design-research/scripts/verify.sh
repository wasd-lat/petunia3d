...
SKILL_DIR="src/prumo/resources/workforce/skills/design-research"

echo "[Prumo Skill: design-research] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/heuristic-evaluation.md" \
  "templates/wireframe-spec.md" \
  "checks/design-research-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "design-research" <<'PY'
import json
import os
import sys
...
check_grep "Reference|evidence" "SKILL.md" "skill requires traceable research references"
check_grep "inference|observation" "SKILL.md" "skill separates evidence from inference"
check_grep "pattern|friction" "SKILL.md" "skill extracts patterns and friction"
check_grep "Usability Heuristics|Error prevention" "references/heuristic-evaluation.md" "reference defines a usability rubric"
check_grep "Evidence Quality|Recommendations" "checks/design-research-checklist.md" "checklist covers evidence and recommendations"
check_grep "Merchant Payout|Decision Question" "templates/wireframe-spec.md" "template contains a realistic research plan"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] design-research verification passed"
  exit 0
fi

echo "[ERROR] design-research verification failed with $VIOLATIONS violation(s)"
exit 1
