...
SKILL_DIR="src/prumo/resources/workforce/skills/asset-pipeline"

echo "[Prumo Skill: asset-pipeline] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/asset-pipeline-guide.md" \
  "templates/asset-pipeline-template.md" \
  "checks/asset-pipeline-invariants.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "asset-pipeline" <<'PY'
import json
import os
import sys
...
check_grep "content-addressed|content-hash" "SKILL.md" "skill requires content-addressed artifacts"
check_grep "hot reload|Hot Reload" "SKILL.md" "skill covers safe hot reload"
check_grep "stream|fallback" "SKILL.md" "skill covers streaming and fallbacks"
check_grep "artifact_key|Deterministic Cooking" "references/asset-pipeline-guide.md" "reference defines deterministic cooking"
check_grep "determin|budget" "checks/asset-pipeline-invariants.md" "checklist covers determinism and budgets"
check_grep "Season 2|Determinism" "templates/asset-pipeline-template.md" "template contains a realistic cook report"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] asset-pipeline verification passed"
  exit 0
fi

echo "[ERROR] asset-pipeline verification failed with $VIOLATIONS violation(s)"
exit 1
