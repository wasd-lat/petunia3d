...
SKILL_DIR="src/prumo/resources/workforce/skills/component-specification"

echo "[Prumo Skill: component-specification] Asset verification"
VIOLATIONS=0
...
for file in \
  "SKILL.md" \
  "manifest.json" \
  "references/component-specification-guide.md" \
  "templates/wireframe-spec.md" \
  "checks/component-specification-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "component-specification" <<'PY'
import json
import os
import sys
...
check_grep "typed props|Props API" "SKILL.md" "skill requires typed prop contracts"
check_grep "variants|states" "SKILL.md" "skill requires a closed state matrix"
check_grep "Keyboard|ARIA" "SKILL.md" "skill requires keyboard and ARIA behavior"
check_grep "Contract First|State Matrix" "references/component-specification-guide.md" "reference defines prop and state contracts"
check_grep "Props and State|Interaction" "checks/component-specification-checklist.md" "checklist covers props and accessibility"
check_grep "DataTable|State Matrix" "templates/wireframe-spec.md" "template contains a realistic component specification"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] component-specification verification passed"
  exit 0
fi

echo "[ERROR] component-specification verification failed with $VIOLATIONS violation(s)"
exit 1
