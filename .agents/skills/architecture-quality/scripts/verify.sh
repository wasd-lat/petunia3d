...
SKILL_DIR="src/prumo/resources/workforce/skills/architecture-quality"

echo "[Prumo Skill: architecture-quality] Asset verification"
VIOLATIONS=0
...
for file in \
  ...
  "templates/architecture-quality-template.md" \
  "checks/architecture-checklist.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "architecture-quality" <<'PY'
import json
import os
import sys
...
check_grep "Dependency Direction|inward" "SKILL.md" "skill enforces inward dependency direction"
check_grep "cycle|God Module" "SKILL.md" "skill covers cycles and oversized modules"
check_grep "fitness function|ArchUnit" "SKILL.md" "skill requires executable architecture checks"
check_grep "Dependency Rule|Layers" "references/clean-architecture.md" "reference defines architectural layers"
check_grep "cycles|boundar" "checks/architecture-checklist.md" "checklist covers cycles and boundaries"
check_grep "Billing Service Extraction|BillingService" "templates/architecture-quality-template.md" "template contains a realistic boundary review"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] architecture-quality verification passed"
  exit 0
fi

echo "[ERROR] architecture-quality verification failed with $VIOLATIONS violation(s)"
exit 1
