...
SKILL_DIR="src/prumo/resources/workforce/skills/game-engine-architecture"

echo "[Prumo Skill: game-engine-architecture] Asset verification"
VIOLATIONS=0
...
for file in \
  ...
  "templates/game-engine-architecture-template.md" \
  "checks/game-engine-architecture-invariants.md" \
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if ! python3 - "$SKILL_DIR" "game-engine-architecture" <<'PY'
import json
import os
import sys
...
check_grep "ECS|Component" "SKILL.md" "skill covers data-oriented ECS architecture"
check_grep "fixed timestep|deterministic" "SKILL.md" "skill requires deterministic stepping"
check_grep "task graph|pool" "SKILL.md" "skill covers scheduling and memory pooling"
check_grep "Archetypes|Sparse Sets" "references/ecs_patterns.md" "reference compares ECS storage strategies"
check_grep "Determinism|Performance" "checks/game-engine-architecture-invariants.md" "checklist covers determinism and budgets"
check_grep "ECS Scheduler|Task graph" "templates/game-engine-architecture-template.md" "template contains a realistic engine review"

if [ "$VIOLATIONS" -eq 0 ]; then
  echo "[SUCCESS] game-engine-architecture verification passed"
  exit 0
fi

echo "[ERROR] game-engine-architecture verification failed with $VIOLATIONS violation(s)"
exit 1
