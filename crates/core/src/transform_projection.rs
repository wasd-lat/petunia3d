...
pub fn plane_point(
    ...
    if !viewport.is_finite() || viewport.min_element() <= 0.0 || !pixel.is_finite() {
        return None;
    }
...
pub fn plane_delta(
    ...
    Some(
        plane_point(camera, viewport, current, pivot, normal)?
            - plane_point(camera, viewport, start, pivot, normal)?,
    )
}
