...
impl<V: PetuniaViewport> SlintUiBridge<V> {
    ...
    pub fn execute_command(&mut self, id: CommandId) {
        ...
        match id {
            ...
            CommandId::UvUnwrap => {
                match petunia_module_uv::UvModule::unwrap_auto(&mut self.state) {
                    Ok(islands) => self
                        .state
                        .set_status(format!("Unwrapped {islands} UV islands")),
                    Err(error) => self.state.set_status(error),
                }
            }
            CommandId::UvPackIslands => {
                petunia_module_uv::UvModule::reproject(&mut self.state);
                self.state.set_status("UV layout projected and normalized");
            }
